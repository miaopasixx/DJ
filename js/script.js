                ...
                ${lnkFiles.map(file => {
                    ...
                    return `
                        <div style="break-inside: avoid; margin-bottom: 10px; position: relative;">
                            <img src="img/folder-icon.svg" style="width: 48px; height: 48px; border-radius: 5px;">
                            <h4 style="color: white; background: rgba(0, 0, 0, 0.5); padding: 2px 5px; border-radius: 3px; text-align: center; margin-top: 5px; cursor: pointer; text-decoration: none; width: auto;" onclick="copyToClipboard('${filePath}')">${fileObj.name}</h4>
                        </div>
                    `;
                }).join('')}
            </div>
